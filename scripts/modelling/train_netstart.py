...
def load_and_partition_data(model_no):
    ...
    Returns:
        ...
        data_test (df): dataframe with test data
    """
...
def encode_nucleotide_to_amino_acid(sequence):
    ...
    Returns:
        amino_acid_sequence (str): An amino acid sequence
    """
...
def extract_datasets(data_train, 
    ...
    Returns:
        ...
        df_test (df): dataframe with test data
    """
...
def one_hot_encode(sequence):
    ...
    Returns: 
        encoding (torch.tensor): one-hot encoded sequence
    """
...
def create_encodings(df_train, df_val, df_test, extract_upstream_aa, extract_downstream_aa):
    ...
    Returns:
        labels_train (np.array): encoded labels for training data
        train_encodings_aa (dict): encoded amino acid sequences for training data
        sequences_train_nt (torch.tensor): one-hot encoded nucleotide sequences for training data
        labels_val (np.array): encoded labels for validation data
        val_encodings_aa (dict): encoded amino acid sequences for validation data
        sequences_val_nt (torch.tensor): one-hot encoded nucleotide sequences for validation data
        labels_test (np.array): encoded labels for test data
        test_encodings_aa (dict): encoded amino acid sequences for test data
        sequences_test_nt (torch.tensor): one-hot encoded nucleotide sequences for test
    """
...
class MultiInputDataset(torch.utils.data.Dataset):
    ...
    Args:
        aa_encodings (dict): encoded amino acid sequences
        nt_encodings (torch.tensor): one-hot encoded nucleotide sequences
        tax_ranks (np.array): encoded taxonomical ranks
        group (np.array): encoded group
...
class NetstartModel(nn.Module):
    ...
    def forward(self, 
        ...
        Args:
            x_aa (dict): amino acid encodings
            x_nt (torch.tensor): nucleotide encodings
            attention_mask_aa (torch.tensor): attention mask for amino acid encodings
            tax_ranks (torch.tensor): taxonomical ranks
        """
...
for epoch in range(num_epochs):
    ...
    for batch in train_loader:
        ...
        if step % int(steps_between_evals/4) == 0:
            print(f"Step {step} / {int(steps_per_epoch) * num_epochs} (Total steps: {int(steps_per_epoch) * num_epochs})", sep = " ", flush=True)
        ...
        if step % steps_between_evals == 0:
            ...
            with torch.no_grad():
                ...
                if val_avg_loss < best_val_loss:
                    ...
                    if isinstance(netstart_model, (torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel)):
                        state_dict = netstart_model.module.state_dict()  # Strips 'module.' prefix
                    else:
                        state_dict = netstart_model.state_dict()

                    # Save the model parameters
                    torch.save(state_dict, '../../data/data_model/models/' + netstart_model_filename + ".pth")
                else:
                    counter_patience += 1
...
TP = sum((test_true_labels == 1.0) & (test_predicted_probs >= best_threshold) for test_true_labels, test_predicted_probs in zip(test_true_labels, test_predicted_probs))
FN = sum((test_true_labels == 1.0) & (test_predicted_probs < best_threshold) for test_true_labels, test_predicted_probs in zip(test_true_labels, test_predicted_probs))
sensitivity = TP / (TP + FN)

TN = sum((test_true_labels == 0.0) & (test_predicted_probs < best_threshold) for test_true_labels, test_predicted_probs in zip(test_true_labels, test_predicted_probs))
FP = sum((test_true_labels == 0.0) & (test_predicted_probs >= best_threshold) for test_true_labels, test_predicted_probs in zip(test_true_labels, test_predicted_probs))
