...
def encode_nucleotide_to_amino_acid(sequence):
    ...
    Returns:
        amino_acid_sequence (str): A string representing the amino acid sequence translated from the 
                                   ...
                                   with unknown or stop codons represented by "<unk>".
    """
    ...
    with open_func(filepath, 'rt') as file:
        ...
        for line in file:
            ...
            if line.startswith(">"):  # Identify header lines
                if header:  # Yield previous entry if exists
                    yield (header, ''.join(sequence))
                header = line[1:]  # Remove ">" from header
                sequence = []  # Reset sequence list for the new entry
            else:
                sequence.append(line)  # Accumulate sequence lines
...
def get_formatted_nucleotide_sequence(sequence, position, nucleotides_upstream, nucleotides_downstream, extract_upstream_nt, extract_downstream_nt):
    """
    ...
    Args: 
        ...
        nucleotides_upstream (int): The position of the targeted ATG in sequence.
    ...
    assert position == nucleotides_upstream ##???Check if position and nucleotides_upstream is the same 
                
    #If needed to extract more nucleotides than there is in sequence upstream
    if nucleotides_upstream < extract_upstream_nt: 
        pad_positions_nt_upstream = extract_upstream_nt - nucleotides_upstream
...
def extract_datasets_nt(input_filename, 
    ...
    for header, sequence in read_fasta(input_filename):
        ...
        for position in positions:
            ...
            sequence_nt = get_formatted_nucleotide_sequence(sequence, position, nucleotides_upstream, nucleotides_downstream, extract_upstream_nt, extract_downstream_nt)
...
def create_encodings_aa(df_input, extract_upstream_aa, extract_downstream_aa, batch_size=32):
    """
...
def ExtractDataAndModel(model_no, input_filename, aa_encodings_len, vocab_sizes):
    """
    ...
    dataloader_nt = DataLoader(dataset_nt, batch_size=128, shuffle=False)
...
def get_predictions(origin, input_filename, output_filename, output_results, threshold):
    ...
    print(f"\nProcessing time: {total_time:.2f} seconds ({total_time/60:.2f} minutes)")
    ...
    total_batches = min(len(test_loader1), len(test_loader2), len(test_loader3), len(test_loader4))
